  ...
  return (
    <div ref={ref} className={className}>
      {Array.isArray(children) ? children.map((child, i) => (
        <div key={i} className={`stagger-item ${visible ? 'visible' : ''}`} style={{ transitionDelay: `${i * 0.12}s` }}>{child}</div>
      )) : children}
    </div>
  );
}
...
function MagneticButton({ children, className, onClick, href }) {
  ...
  const style = { transform: `translate(${offset.x}px, ${offset.y}px)`, transition: 'transform 0.3s cubic-bezier(0.23,1,0.32,1)' };

  if (href) {
    return (
      <Link href={href} ref={btnRef} className={className} onMouseMove={handleMove} onMouseLeave={() => setOffset({ x: 0, y: 0 })} style={style}>
        {children}
      </Link>
    );
  }
  return (
    <button ref={btnRef} className={className} onClick={onClick} onMouseMove={handleMove} onMouseLeave={() => setOffset({ x: 0, y: 0 })} style={style}>
      {children}
    </button>
  );
}
...
const content = {
  en: {
    ...
    values: {
      ...
      items: [
        ...
        { icon: '\ud83e\udd1d', title: 'Partnership', desc: 'We embed ourselves in your team. Your challenges become our challenges, and your success is our success.' },
    ...
    numbers: {
      ...
      items: [
        { value: '12+', label: 'Years of experience in data & analytics' },
        { value: '40+', label: 'Countries where we\'ve delivered solutions' },
        { value: '300%', label: 'Average revenue growth for our clients' },
        { value: '14+', label: 'Major telecom monetization projects' },
      ],
    },
    story: {
      label: 'OUR STORY',
  ...
  hr: {
    ...
    values: {
      ...
      title: '\u0160to nas pokrec\u0301e',
      items: [
        { icon: '\u26a1', title: 'Inovacija na prvom mjestu', desc: 'Ostajemo ispred krivulje, konstantno istra\u017euju\u0107i nove AI tehnologije i metodologije.' },
        { icon: '\ud83c\udfaf', title: 'Vo\u0111eni podacima', desc: 'Svaka odluka, svaka strategija, svako rje\u0161enje koje gradimo ukorijenjeno je u podacima.' },
        { icon: '\ud83e\udd1d', title: 'Partnerstvo', desc: 'Ugra\u0111ujemo se u va\u0161 tim. Va\u0161i izazovi postaju na\u0161i izazovi, a va\u0161 uspjeh je na\u0161 uspjeh.' },
    ...
    <>
      ...
      <section className="page-hero">
        ...
        <div className="page-hero-content">
          <LineReveal>
            <p className="page-hero-label">{t.hero.label}</p>
          </LineReveal>
        ...
        <div className="about-mission-inner">
          <div>
            ...
            <h2 className="section-title">
              <SplitText>{t.mission.title}</SplitText>
            </h2>
          </div>
      ...
      <section className="about-values">
        ...
        <h2 className="section-title">
          <SplitText>{t.values.title}</SplitText>
        </h2>
      ...
      <section className="about-numbers" ref={numbersRef}>
        <div className="about-numbers-inner">
          <LineReveal>
            <p className="section-label">{t.numbers.label}</p>
          </LineReveal>
          <h2 className="section-title">
            <SplitText>{t.numbers.title}</SplitText>
          </h2>
      ...
      <section className="about-story" ref={storyRef}>
        ...
        <h2 className="section-title">
          <SplitText>{t.story.title}</SplitText>
        </h2>
      ...
      <section className="about-cta">
        ...
        <div className="about-cta-inner">
          <h2 className="about-cta-title">
            <SplitText center>{t.cta.title}</SplitText>
          </h2>
          <LineReveal delay={0.2}>
            <p className="about-cta-sub">{t.cta.sub}</p>
          </LineReveal>
