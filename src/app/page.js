...
function ScrollProgress() { const [p, setP] = useState(0); useEffect(() => { const u = () => { const s = window.scrollY; const d = document.documentElement.scrollHeight - window.innerHeight; setP(d > 0 ? (s / d) * 100 : 0); }; window.addEventListener('scroll', u, { passive: true }); return () => window.removeEventListener('scroll', u); }, []); return <div className="scroll-progress" style={{ width: `${p}%` }} />; }
function CustomCursor() { const c = useRef(null); const d = useRef(null); const l = useRef(null); const pos = useRef({ x: -100, y: -100 }); const tgt = useRef({ x: -100, y: -100 }); const [h, setH] = useState(false); const [lb, setLb] = useState(''); useEffect(() => { const mv = (e) => { tgt.current = { x: e.clientX, y: e.clientY }; }; const an = () => { pos.current.x += (tgt.current.x - pos.current.x) * 0.15; pos.current.y += (tgt.current.y - pos.current.y) * 0.15; if (c.current) { c.current.style.left = `${pos.current.x}px`; c.current.style.top = `${pos.current.y}px`; } if (d.current) { d.current.style.left = `${tgt.current.x}px`; d.current.style.top = `${tgt.current.y}px`; } if (l.current) { l.current.style.left = `${pos.current.x}px`; l.current.style.top = `${pos.current.y + 35}px`; } requestAnimationFrame(an); }; window.addEventListener('mousemove', mv); requestAnimationFrame(an); const ah = () => { document.querySelectorAll('a, button, .service-card, .tech-category').forEach(el => { el.addEventListener('mouseenter', () => { setH(true); setLb(el.dataset.cursorLabel || ''); }); el.addEventListener('mouseleave', () => { setH(false); setLb(''); }); }); }; ah(); const obs = new MutationObserver(ah); obs.observe(document.body, { childList: true, subtree: true }); return () => { window.removeEventListener('mousemove', mv); obs.disconnect(); }; }, []); return (<><div ref={c} className={`custom-cursor ${h ? 'hovering' : ''}`} /><div ref={d} className={`custom-cursor-dot ${h ? 'hovering' : ''}`} /><div ref={l} className={`custom-cursor-label ${lb ? 'visible' : ''}`}>{lb}</div></>); }
function ParallaxHero({ children }) { const hr = useRef(null); const cr = useRef(null); const mr = useRef(null); useEffect(() => { const hs = () => { const s = window.scrollY; const hh = hr.current?.offsetHeight || 1; if (s > hh) return; const r = s / hh; if (cr.current) { cr.current.style.transform = `translateY(${s * 0.3}px)`; cr.current.style.opacity = `${1 - r * 1.2}`; } if (mr.current) { mr.current.style.transform = `translateY(${s * -0.15}px) scale(${1 + r * 0.1})`; } }; window.addEventListener('scroll', hs, { passive: true }); return () => window.removeEventListener('scroll', hs); }, []); return (<section className="hero" ref={hr}><div className="hero-mesh" ref={mr}><div className="hero-mesh-orb" /><div className="hero-mesh-orb" /><div className="hero-mesh-orb" /><div className="hero-mesh-orb" /><div className="hero-mesh-orb" /></div><div className="hero-grid" /><div className="hero-ring" /><ParticleCanvas /><div className="hero-horizon" /><div ref={cr} style={{ position: 'relative', zIndex: 3, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>{children}</div></section>); }
function ParticleCanvas() { const cv = useRef(null); const an = useRef(null); const ms = useRef({ x: -1000, y: -1000 }); useEffect(() => { const canvas = cv.current; if (!canvas) return; const ctx = canvas.getContext('2d'); let w, h, pts; const rs = () => { w = canvas.width = canvas.offsetWidth; h = canvas.height = canvas.offsetHeight; }; const init = () => { rs(); const cnt = Math.min(Math.floor((w * h) / 10000), 90); pts = Array.from({ length: cnt }, () => ({ x: Math.random() * w, y: Math.random() * h, vx: (Math.random() - 0.5) * 0.3, vy: (Math.random() - 0.5) * 0.3, r: Math.random() * 1.5 + 0.3 })); }; const dr = () => { ctx.clearRect(0, 0, w, h); const mx = ms.current.x, my = ms.current.y; for (let i = 0; i < pts.length; i++) { const p = pts[i]; p.x += p.vx; p.y += p.vy; if (p.x < 0) p.x = w; if (p.x > w) p.x = 0; if (p.y < 0) p.y = h; if (p.y > h) p.y = 0; const dM = Math.hypot(p.x - mx, p.y - my); const gl = dM < 180 ? 1 - dM / 180 : 0; ctx.beginPath(); ctx.arc(p.x, p.y, p.r + gl * 2.5, 0, Math.PI * 2); ctx.fillStyle = `rgba(0,194,203,${0.2 + gl * 0.8})`; ctx.fill(); for (let j = i + 1; j < pts.length; j++) { const q = pts[j]; const d = Math.hypot(p.x - q.x, p.y - q.y); if (d < 120) { ctx.beginPath(); ctx.moveTo(p.x, p.y); ctx.lineTo(q.x, q.y); ctx.strokeStyle = `rgba(0,194,203,${0.06 * (1 - d / 120)})`; ctx.lineWidth = 0.5; ctx.stroke(); } } } an.current = requestAnimationFrame(dr); }; init(); dr(); window.addEventListener('resize', rs); const hm = (e) => { const rect = canvas.getBoundingClientRect(); ms.current = { x: e.clientX - rect.left, y: e.clientY - rect.top }; }; canvas.addEventListener('mousemove', hm); return () => { cancelAnimationFrame(an.current); window.removeEventListener('resize', rs); canvas.removeEventListener('mousemove', hm); }; }, []); return <canvas ref={cv} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', pointerEvents: 'auto', zIndex: 2 }} />; }
...
function ContactForm({ t }) {
  ...
  const btnClass = `submit-btn ${status === 'sent' ? 'sent' : ''} ${status === 'error' ? 'error' : ''}`;
  ...
  return (
    <div className="contact-form">
      ...
      <MagneticButton className={btnClass} onClick={handleSubmit} disabled={status === 'sending'}>{btnText}</MagneticButton>
    </div>
  );
}
    ...
    <footer className="big-footer">
      ...
      <div className="big-footer-inner">
        ...
        <div className="big-footer-top">
          <div className="big-footer-brand"><div className="big-footer-logo">Dom<span>Analytics</span></div><p className="big-footer-desc">{t.footer.desc}</p><div className="big-footer-social"><a href="https://linkedin.com/in/domagojkrusic" target="_blank" rel="noopener noreferrer">in</a><a href="mailto:info@domanalytics.com">@</a></div></div>
          ...
          <div className="big-footer-col"><h4>{t.footer.connect}</h4><a href="mailto:info@domanalytics.com">info@domanalytics.com</a><a href="https://linkedin.com/in/domagojkrusic" target="_blank" rel="noopener noreferrer">LinkedIn</a></div>
...
export default function Home() {
  ...
  return (
    <>
      <div className="noise" /><ScrollProgress /><CustomCursor />
